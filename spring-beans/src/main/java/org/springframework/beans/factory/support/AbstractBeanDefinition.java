...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * 默认scope 枚举 是个空串,最终会被匹配为Singleton
	 */
	...
	@Nullable
	/**
	 * BeanDefinition 对应的Bean类型,这个值可能为class
	 * 或String,在解析形成sourceClass时
	 * 如果是 String 类型 使用ASM 解析形成 SimpleAnnotationMetadata
	 * 如果是 Class 类型 通过工具类直接创建 StandardAnnotationMetadata
	 */
	private volatile Object beanClass;

	@Nullable
	/**
	 * 作用域 scope Default 处理时当成singleton 处理
	 */
	...
	@Nullable
	/**
	 * 对象提供者,当 instanceSupplier 不为空时,不走Bean创建而是直接从instanceSupplier获取
	 * instanceSupplier 同样会走populateBean 和 initializeBean
	 */
	private Supplier<?> instanceSupplier;

	/**
	 * 对非public 的构造方法的限制,
	 * 当且仅当 beanClass 不为空 且当前类不是public 且nonPublicAccessAllowed 为false 时,不可以访问非 public 的构造方法
	 * 也就是说当前属性无法禁止public class
	 */
